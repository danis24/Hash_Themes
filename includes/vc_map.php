...
class WPBakeryShortCode_Sh_Pricing_Table extends WPBakeryShortCodesContainer {
}
class WPBakeryShortCode_Sh_Table extends WPBakeryShortCode {
}
...
function Hash_custom_css_classes_for_vc_row_and_vc_column($class_string, $tag) {
...
function vc_theme_vc_row($atts, $content = null) {
   ...
   extract(shortcode_atts(array(
	...
	$boxed = hash_wow_sh_set( HASH_WSH()->option(), 'boxed_layout' );
	
	if( $container && !$boxed ) return
...
function vc_theme_vc_row_inner($atts, $content = null) {
   ...
   extract(shortcode_atts(array(
	...
	'<section class="'.$css_class.'" '.$style.' >
 	...
 	return 
	'<section class="'.$css_class.' general-row" >
		'.wpb_js_remove_wpautop($content).'
	</section>'."\n";  
}
...
$param = array(
  ...
  "holder" => "div",
  "class" => "",
  "heading" => esc_html__("Container", 'hash'),
  "param_name" => "container",
  "value" => array('Center Page'=>true),
  "description" => esc_html__("Choose whether you want to add a container before row or not.", 'hash')
);
